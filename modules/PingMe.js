...
function buildHeaders(capture, ua) {
  const out = {};
  Object.keys(capture.headers || {}).forEach(k => out[k] = capture.headers[k]);
  delete out['Content-Length']; delete out['content-length'];
  delete out[':authority']; delete out[':method']; delete out[':path']; delete out[':scheme'];
  out['Host'] = 'api.pingmeapp.net';
  out['Accept'] = out['Accept'] || 'application/json';
  Object.keys(out).forEach(k => { if (k.toLowerCase() === 'user-agent') delete out[k]; });
  out['User-Agent'] = ua;
  return out;
}

// ===================== Surge HTTP 请求封装 =====================
// 统一请求函数，同时支持 http-request（内置$http不等）和 cron（需自行调用）
...
// ===================== 账号任务执行 =====================
function runAccount(acc, index, total, fetchFn) {
  const tag = `[账号${index+1}/${total} ${acc.alias || acc.id}]`;
  const ua = buildUA(acc.baseUA, acc.uaSeed);
  const headers = buildHeaders(acc.capture, ua);
  const msgs = [tag];
  console.log(`【${scriptName}】开始执行账号 ${tag}`);
  ...
  function doVideoLoop(count) {
    ...
    function next() {
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          i++;
          console.log(`【${scriptName}】${tag} 视频${i}/${count} 请求中...`);
          fetchFn(buildUrl('videoBonus', acc.capture), headers).then(res => {
            try {
              const d = JSON.parse(res.body);
              if (d.retcode === 0) {
                msgs.push(`🎬 视频${i}：+${d.result?.bonus || '?'} Coins`);
                console.log(`【${scriptName}】${tag} 视频${i} 成功`);
                resolve(next());
              } else {
                msgs.push(`⏸ 视频${i}：${d.retmsg}`);
                console.log(`【${scriptName}】${tag} 视频${i} 跳过：${d.retmsg}`);
                resolve();
              }
            } catch (e) {
              msgs.push(`❌ 视频${i}：解析失败`);
              console.log(`【${scriptName}】${tag} 视频${i} 解析失败`);
              resolve();
            }
          }).catch(err => {
            msgs.push(`❌ 视频${i}：${err.error || String(err)}`);
            console.log(`【${scriptName}】${tag} 视频${i} 请求失败：${err.error || String(err)}`);
            resolve(next());
          });
  ...
  console.log(`【${scriptName}】${tag} 查询余额...`);
  return fetchFn(buildUrl('queryBalanceAndBonus', acc.capture), headers).then(res => {
    try {
      const d = JSON.parse(res.body);
      if (d.retcode === 0) {
        msgs.push(`💰 余额：${d.result.balance} Coins`);
        console.log(`【${scriptName}】${tag} 余额：${d.result.balance} Coins`);
      } else {
        msgs.push(`⚠️ 查询：${d.retmsg}`);
        console.log(`【${scriptName}】${tag} 查询失败：${d.retmsg}`);
      }
    } catch (e) { msgs.push('❌ 查询：解析失败'); }
    console.log(`【${scriptName}】${tag} 签到中...`);
    return fetchFn(buildUrl('checkIn', acc.capture), headers);
  }).then(res => {
    try {
      const d = JSON.parse(res.body);
      if (d.retcode === 0) {
        msgs.push(`✅ 签到：${(d.result?.bonusHint || d.retmsg || '').replace(/\n/g, ' ')}`);
        console.log(`【${scriptName}】${tag} 签到成功`);
      } else {
        msgs.push(`⚠️ 签到：${d.retmsg}`);
        console.log(`【${scriptName}】${tag} 签到失败：${d.retmsg}`);
      }
    } catch (e) { msgs.push('❌ 签到：解析失败'); }
    console.log(`【${scriptName}】${tag} 开始看视频（共${MAX_VIDEO}个）...`);
    return doVideoLoop(MAX_VIDEO);
  }).then(() => {
    console.log(`【${scriptName}】${tag} 查询最新余额...`);
    return fetchFn(buildUrl('queryBalanceAndBonus', acc.capture), headers);
  }).then(res => {
    try {
      const d = JSON.parse(res.body);
      if (d.retcode === 0) {
        msgs.push(`💰 最新余额：${d.result.balance} Coins`);
        console.log(`【${scriptName}】${tag} 最新余额：${d.result.balance} Coins`);
      }
    } catch (e) {}
    ...
    console.log(`【${scriptName}】${tag} 异常：${err.error || String(err)}`);
    return msgs.join('\n');
  });
}
...
if (typeof $request !== 'undefined' && $request && $request.url) {
  // ── http-request 入口（账号抓取）───────────────────────
  const paramsRaw = parseRawQuery($request.url);
  let baseUA = '';
  Object.keys($request.headers || {}).forEach(k => {
    if (k.toLowerCase() === 'user-agent') baseUA = $request.headers[k];
  });
  ...
  if (!ids.length) {
    console.log(`【${scriptName}】未抓到任何账号`);
    notify('⚠️ PingMe 未抓到任何账号', '请先打开 PingMe 触发抓包', '');
    $done();
  }
  ...
  ids.forEach((id, idx) => {
    chain = chain.then(() => runAccount(store.accounts[id], idx, total, httpGet))
      .then(text => { results.push(text); })
      .then(() => idx < ids.length - 1 ? sleep(ACCOUNT_GAP) : null);
  });

  chain.then(() => {
    notify(`🎉 PingMe 全部完成 (${total}个账号)`, results.join('\n———\n'), '');
    $done();
  }).catch(err => {
    notify('❌ PingMe 任务异常', results.join('\n———\n') + '\n' + (err.error || String(err)), '');
    $done();
  });
}
